...
def gp02_calculation(fails, passes, total_failed_tests, total_passes_tests):
    npasses = total_passes_tests - passes
    return 2*(fails + math.sqrt(npasses))+math.sqrt(passes)

def gp03_calculation(fails, passes, total_failed_tests, total_passes_tests):
    return math.sqrt(abs(fails*fails - math.sqrt(passes)))

def gp19_calculation(fails, passes, total_failed_tests, total_passes_tests):
    npasses = total_passes_tests - passes
    nfails = total_passes_tests - passes
    return fails*math.sqrt(abs(passes - fails + nfails -npasses))

def suspicious_score_by_sbfl_metric(spectrum_expression, fails, passes, total_failed_tests, total_passed_tests):
    if spectrum_expression == TARANTULA:
        return tarantula_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == OCHIAI:
        return ochiai_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == OP2:
        return op2_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == BARINEL:
        return barinel_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == DSTAR:
        return dstar_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == RUSSELL_RAO:
        return russell_rao_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == SIMPLE_MATCHING:
        return simple_matching_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == ROGERS_TANIMOTO:
        return rogers_tanimoto_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == AMPLE:
        return ample2_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == JACCARD:
        return jaccard_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == COHEN:
        return cohen_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == SCOTT:
        return scott_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == ROGOT1:
        return rogot1_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == GEOMETRIC_MEAN:
        return geometric_mean_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == M2:
        return m2_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == WONG1:
        return wong1_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == SOKAL:
        return sokal_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == SORENSEN_DICE:
        return sorensen_dice_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == DICE:
        return dice_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == HUMANN:
        return humman_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == M1:
        return m1_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == WONG2:
        return wong2_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == WONG3:
        return wong3_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == ZOLTAR:
        return zoltar_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == OVERLAP:
        return overlap_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == EUCLID:
        return euclid_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == ROGOT2:
        return rogot2_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == HAMMING:
        return hamming_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == FLEISS:
        return fleiss_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == ANDERBERG:
        return anderberg_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == GOODMAN:
        return goodman_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == HARMONIC_MEAN:
        return harmonic_mean_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == KULCZYNSKI1:
        return kulczynski1_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == KULCZYNSKI2:
        return kulczynski2_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == GP02:
        return gp02_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == GP03:
        return gp03_calculation(fails, passes, total_failed_tests, total_passed_tests)
    elif spectrum_expression == GP19:
        return gp19_calculation(fails, passes, total_failed_tests, total_passed_tests)
