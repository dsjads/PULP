...
def remove_src_code_file(system_paths):
    for system in system_paths:
        for bug in system_paths[system]:
            ...
            for mutated_project in mutated_projects:
                ...
                variants_path = get_variants_dir(mu_project_path)
                variants_projects = list_dir(variants_path)
                for variants_project in variants_projects:
                    variants_build_paths = join_path(variants_path, variants_project, "coverage")
                    print(variants_build_paths)
                    if os.path.exists(variants_build_paths):
                        remove_folder(variants_build_paths)
